...
public class InterruptionActivity extends Activity {
    LinearLayout root;CountDownTimer timer;InteractionSpec spec;MediaRecorder recorder;File audioFile;int revealOrder;
    @Override public void onCreate(Bundle b){super.onCreate(b);getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON|WindowManager.LayoutParams.FLAG_SHOW_WHEN_LOCKED|WindowManager.LayoutParams.FLAG_TURN_SCREEN_ON);loadSpec();PremiumUi.window(this,spec.attention()==Attention.URGENT);if(spec.readyGate())showReady();else showInteraction();}
    ...
    Button b(String s,boolean primary){Button x=new Button(this);x.setText(s);x.setTextSize(20);x.setAllCaps(false);x.setMinHeight(112);PremiumUi.button(x,primary);return x;}
    void add(Button x){root.addView(x);final int o=revealOrder++;x.post(()->PremiumUi.reveal(x,o));}
    void base(String prompt){revealOrder=0;root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setGravity(Gravity.CENTER);root.setPadding(36,36,36,36);root.setBackgroundColor(spec.attention()==Attention.URGENT?0xff160b12:0xff101218);TextView t=new TextView(this);t.setText(prompt);t.setTextSize(30);t.setTypeface(Typeface.DEFAULT_BOLD);t.setGravity(Gravity.CENTER);PremiumUi.heading(t);root.addView(t,new LinearLayout.LayoutParams(-1,0,1));t.post(()->PremiumUi.reveal(t,0));setContentView(root);}
    void showReady(){base(spec.readyPrompt());Button yes=b("READY — SHOW IT",true);yes.setOnClickListener(v->showInteraction());add(yes);Button snooze=b("SNOOZE",false);snooze.setOnClickListener(v->finish());add(snooze);}
    void addDeadline(){if(spec.countdownMs()<=0)return;TextView d=new TextView(this);d.setTextSize(18);d.setTextColor(0xfff0b9c9);d.setGravity(Gravity.CENTER);d.setPadding(0,0,0,10);root.addView(d,0);timer=new CountDownTimer(spec.countdownMs(),100){public void onTick(long m){d.setText("ANSWER — "+String.format("%.1f",m/1000.0)+"s");}public void onFinish(){d.setText("ANSWER NOW");}};timer.start();}
    void showInteraction(){base(spec.prompt());addDeadline();if(spec.capture()==Capture.CHOICE){int i=0;for(Choice c:spec.choices()){Button q=b(c.label(),i++==0);q.setOnClickListener(v->sendChoice(c.id()));add(q);}}else if(spec.capture()==Capture.AUDIO){showAudioControls();}else{Button done=b("DONE",true);done.setOnClickListener(v->{postSimple("done");finish();});add(done);}Button snooze=b("SNOOZE",false);snooze.setOnClickListener(v->{postSimple("snooze");finish();});add(snooze);Button stop=b("STOP",false);stop.setOnClickListener(v->{postSimple("stop");finish();});add(stop);}
