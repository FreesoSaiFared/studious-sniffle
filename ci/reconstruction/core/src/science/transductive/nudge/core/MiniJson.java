...
public final class MiniJson {
    ...
    private static final class Parser{
        ...
        Map<String,Object> object(){i++;ws();Map<String,Object>m=new LinkedHashMap<String,Object>();if(peek('}')){i++;return m;}while(true){ws();if(!peek('"'))throw err("expected key");String k=string();ws();expect(':');if(m.containsKey(k))throw err("duplicate key");Object v=value();m.put(k,v);ws();if(peek('}')){i++;return m;}expect(',');}}
        ...
        String string(){expect('"');StringBuilder b=new StringBuilder();while(!eof()){char c=s.charAt(i++);if(c=='"')return b.toString();if(c!='\\'){if(c<0x20)throw err("raw control in string");b.append(c);continue;}if(eof())throw err("bad escape");char e=s.charAt(i++);switch(e){case '"':b.append('"');break;case '\\':b.append('\\');break;case '/':b.append('/');break;case 'b':b.append('\b');break;case 'f':b.append('\f');break;case 'n':b.append('\n');break;case 'r':b.append('\r');break;case 't':b.append('\t');break;case 'u':if(i+4>s.length())throw err("bad unicode");b.append((char)Integer.parseInt(s.substring(i,i+4),16));i+=4;break;default:throw err("bad escape");}}throw err("unterminated string");}
        Object number(){int st=i;if(peek('-'))i++;while(!eof()&&Character.isDigit(s.charAt(i)))i++;boolean fp=false;if(peek('.')){fp=true;i++;int ds=i;while(!eof()&&Character.isDigit(s.charAt(i)))i++;if(i==ds)throw err("fraction requires digit");}if(!eof()&&(s.charAt(i)=='e'||s.charAt(i)=='E')){fp=true;i++;if(!eof()&&(s.charAt(i)=='+'||s.charAt(i)=='-'))i++;while(!eof()&&Character.isDigit(s.charAt(i)))i++;}if(st==i)throw err("bad number");String n=s.substring(st,i);if(fp)return Double.valueOf(n);return Long.valueOf(n);}
