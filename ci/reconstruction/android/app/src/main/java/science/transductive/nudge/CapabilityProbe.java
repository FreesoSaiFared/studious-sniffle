...
public final class CapabilityProbe {
    ...
    public static boolean accessibilityConnected(){return NudgeAccessibilityService.current()!=null;}
    public static boolean displayScreenshotApi(){return Build.VERSION.SDK_INT>=30;}
    public static boolean windowScreenshotApi(){return Build.VERSION.SDK_INT>=34;}
    public static boolean targetLaunchable(Context c,String pkg){return pkg!=null&&c.getPackageManager().getLaunchIntentForPackage(pkg)!=null;}
    public static String summary(Context c){return "notifications="+notifications(c)+"\nfullScreenIntent="+fullScreen(c)+"\nexactAlarm="+exactAlarm(c)+"\nnotificationPolicy="+policy(c)+"\nmicrophone="+microphone(c)+"\naccessibilityGranted="+accessibility(c)+"\naccessibilityConnected="+accessibilityConnected()+"\nscreenshotApi="+displayScreenshotApi()+"\nwindowScreenshotApi="+windowScreenshotApi()+"\nchatGptLaunchable="+targetLaunchable(c,"com.openai.chatgpt");}
}
